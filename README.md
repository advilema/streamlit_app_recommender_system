...
You need to have [Python3](https://www.python.org/) first. Follow the instruction corresponding to your operation system.
...
```bash
(venv.scrapy)$ cd recommender_system # go to the same folder as requirement.txt
(venv.scrapy)$ pip install -r requirements.txt
```
...
## The Recommender System app
### Run the app
```bash
(venv.scrapy)$ cd recommender_system/ # stay in the same folder as scrapy.cfg
(venv.scrapy)$ streamlit run app.py
```
...
```
Look at the tutorial below to see how to change the list of possible topics.
...
To change this run:
```bash
$ python initialize.py --model_name THE_MODEL_YOU_LIKE_THE_MOST
```
